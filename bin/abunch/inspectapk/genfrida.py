...
"""
        ...
        if methods.lower() != "no method extracted":
            ...
            for method in method_list:
                method_cleaned = method.split('(')[0].strip()

                if method_cleaned:
                    frida_script += f"""
    try {{
        {class_name.replace('.', '_')}.{method_cleaned}.implementation = function() {{
...
"""
    ...
    try {
        ...
        SSLContext.init.implementation = function(keyManager, trustManager, secureRandom) {
            console.log('[+] Bypassing SSLContext initialization');
            this.init(keyManager, TrustManagers, secureRandom);
        };
    } catch(err) {
        console.log('[-] Certificate pinning bypass failed: ' + err);
    }
