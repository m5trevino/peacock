...
def extract_signatures(decompiled_dir, output_file):
    """
    Extract class and method signatures from decompiled Java files and save them in a single CSV.
    ...
    # Create DataFrame for classes and methods
    classes_df = pd.DataFrame(classes_data)[['class']]
    methods_df = pd.DataFrame(methods_data)[['methods']]
    
    # Save class names and method signatures in a single CSV
    if len(classes_df) == len(methods_df):
        # Save the combined file
        combined_df = pd.concat([classes_df, methods_df], axis=1)
        combined_df.to_csv(output_file, index=False, header=['class', 'methods'])
        print(f"[+] Extracted {len(classes_df)} classes and methods to {output_file}")
        print(f"Total number of methods extracted: {total_methods}")
        print(f"Total number of classes with no methods: {no_method_count}")
    else:
        print("\n[-] Error: The number of classes and methods does not match. Please check the extraction process.")

if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python 2_extract_signatures.py <decompiled_dir> <output_file>")
        print("Example: python 2_extract_signatures.py ./decompiled ./classes_methods.csv")
        sys.exit(1)
    
    decompiled_dir = sys.argv[1]
    output_file = sys.argv[2]
    extract_signatures(decompiled_dir, output_file)
