#!/bin/bash
# 🔧 WORKING PEACOCK BUILDER
# Creates both debug and visual versions

echo "🔧 Building Peacock MCP versions..."

# Create debug version
cat > pea-mcp-debug-working.py << 'EOF'
#!/usr/bin/env python3
"""
...
def display_stage_result(stage_name, success, char_count=0, model="", key_hint=""):
    ...
    debug_log("STAGE", f"{stage_name} {status}", chars=char_count, model=model, key=key_hint)

def display_separator():
    debug_log("STAGE", "--- Stage separator ---")

def display_character_summary(stage_results):
    debug_log("SUMMARY", "Character count summary:")
    total = 0
    for stage, data in stage_results.items():
        chars = data.get("char_count", 0)
        total += chars
        debug_log("SUMMARY", f"  {stage.upper()}: {chars} chars")
    debug_log("SUMMARY", f"TOTAL: {total} characters")
...
class PeacockHandler(BaseHTTPRequestHandler):
    ...
    def execute_peacock_pipeline(self, user_input):
        ...
        try:
            ...
            # Create orchestrator (fix the function call)
...
echo "✅ Created pea-mcp-debug-working.py"
echo ""
echo "🚀 Test it now:"
echo "   python3 pea-mcp-debug-working.py --log"
echo ""
echo "This version has CLEAN debug output - no cyberpunk borders!"
echo "You'll see exactly what character counts are being returned."