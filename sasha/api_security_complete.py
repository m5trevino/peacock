...
class VisualMappingEngine:
    ...
    def generate_visual_maps(self, intelligence_data: Dict, flow_data: Dict, 
        ...
        mobile_summary = self._generate_mobile_summary(intelligence_data, security_findings)
        ...
        return {
            ...
            'export_formats': {
                'svg_exports': self._generate_svg_exports(mermaid_diagrams),
                'json_export': self._generate_json_export(intelligence_data, flow_data, security_findings)
            }
        }
    
    def _generate_api_structure_diagram(self, intelligence_data: Dict) -> str:
        """Generate Mermaid diagram for API structure"""
        endpoint_map = intelligence_data.get('endpoint_map', {})
        high_value_targets = intelligence_data.get('high_value_targets', [])
        ...
        for path, endpoint in endpoint_map.items():
            ...
            domain_groups[service].append((path, endpoint))
        
        # Generate nodes for each service
        for service, endpoints in domain_groups.items():
            service_id = service.replace('-', '_').replace('.', '_')
            diagram_lines.append(f'    {service_id}["{service.title()} Service"]')
            
            for path, endpoint in endpoints:
                endpoint_id = path.replace('/', '_').replace('{', '').replace('}', '').replace('-', '_')
                
                # Color code by security level
                if endpoint.security_level == 'high':
                    style = ':::critical'
                ...
                is_high_value = any(target['path'] == path for target in high_value_targets)
                if is_high_value:
                    style = ':::highvalue'
                
                diagram_lines.append(f'    {endpoint_id}["{path}<br/>{endpoint.method}"] {style}')
                diagram_lines.append(f'    {service_id} --> {endpoint_id}')
        
        # Add styling
        diagram_lines.extend([
            '    classDef critical fill:#ff6b6b,stroke:#d63031,color:#fff',
            '    classDef protected fill:#fdcb6e,stroke:#e17055,color:#000',
            '    classDef public fill:#6c5ce7,stroke:#5f3dc4,color:#fff',
            '    classDef highvalue fill:#00b894,stroke:#00a085,color:#fff'
        ])
        
        return '\n'.join(diagram_lines)
    
    def _generate_auth_flow_diagrams(self, flow_data: Dict) -> Dict[str, str]:
        """Generate authentication flow diagrams"""
        auth_flows = flow_data.get('authentication_flows', [])
        diagrams = {}
        
        for flow in auth_flows:
            flow_id = flow.flow_id
            diagram_lines = ['sequenceDiagram']
            diagram_lines.append('    participant User')
            diagram_lines.append('    participant App')
            diagram_lines.append('    participant API')
            diagram_lines.append('    participant Auth')
            
            for step in flow.steps:
                method = step['method']
                url_parts = step['url'].split('/')
                service = 'Auth' if any(auth_word in step['url'].lower() 
                                      for auth_word in ['auth', 'login', 'token']) else 'API'
                
                if step.get('has_credentials'):
                    diagram_lines.append(f'    User->>App: Enter credentials')
                    diagram_lines.append(f'    App->>Auth: {method} {url_parts[-1]}')
                elif step.get('generates_token'):
                    diagram_lines.append(f'    Auth-->>App: Token response ({step["status"]})')
                    diagram_lines.append(f'    App-->>User: Authentication success')
                else:
                    diagram_lines.append(f'    App->>API: {method} {url_parts[-1] if url_parts else "endpoint"}')
                    diagram_lines.append(f'    API-->>App: Response ({step["status"]})')
            
            diagrams[flow_id] = '\n'.join(diagram_lines)
        
        return diagrams
    
    def _generate_security_overview_diagram(self, security_findings: List[SecurityFinding]) -> str:
        """Generate security overview diagram"""
        # Group findings by category and severity
        finding_stats = defaultdict(lambda: {'critical': 0, 'high': 0, 'medium': 0, 'low': 0})
        
        for finding in security_findings:
            finding_stats[finding.category][finding.severity] += 1
        
        diagram_lines = ['pie title Security Findings Overview']
        
        # Calculate total for each severity
        severity_totals = {'critical': 0, 'high': 0, 'medium': 0, 'low': 0}
        for category_stats in finding_stats.values():
            for severity, count in category_stats.items():
                severity_totals[severity] += count
        
        # Add to pie chart
        for severity, count in severity_totals.items():
            if count > 0:
                diagram_lines.append(f'    "{severity.title()}" : {count}')
        
        return '\n'.join(diagram_lines)
    
    def _generate_data_flow_diagram(self, flow_data: Dict) -> str:
        """Generate data flow diagram"""
        data_flows = flow_data.get('data_flows', {}).get('response_to_request_flow', [])
        ...
        for flow in data_flows:
            source_id = self._sanitize_id(flow['source']['url'])
            target_id = self._sanitize_id(flow['target']['url'])
            
            endpoints.add((source_id, flow['source']['url']))
            endpoints.add((target_id, flow['target']['url']))
            ...
            connections.append(f'    {source_id} {line_style} {target_id}')
        
        # Add endpoint nodes
        for endpoint_id, url in endpoints:
            path = url.split('/')[-1] or 'root'
            diagram_lines.append(f'    {endpoint_id}["{path}"]')
        
        # Add connections
        diagram_lines.extend(connections)
        
        return '\n'.join(diagram_lines)
    
    def _generate_interactive_html_report(self, intelligence_data: Dict, flow_data: Dict, 
                                        security_findings: List[SecurityFinding], 
                                        mermaid_diagrams: Dict) -> str:
        """Generate comprehensive interactive HTML report"""
        
        # Calculate summary statistics
        total_endpoints = len(intelligence_data.get('endpoint_map', {}))
        total_findings = len(security_findings)
        critical_findings = sum(1 for f in security_findings if f.severity == 'critical')
        high_findings = sum(1 for f in security_findings if f.severity == 'high')
...
<head>
    ...
    <style>
        * {{
        ...
        .method-delete {{ background: #8e44ad; color: white; }}
        .method-patch {{ background: #16a085; color: white; }}
...
<body>
    <div class="container">
        <div class="header">
            ...
            <p>Comprehensive security assessment and intelligence overview</p>
            <p><small>Generated on {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</small></p>
        </div>
        
        <div class="stats-grid">
            <div class="stat-card">
                <div class="stat-number good">{total_endpoints}</div>
                <div>Total Endpoints</div>
            </div>
            <div class="stat-card">
                <div class="stat-number {"critical" if critical_findings > 0 else "good"}">{critical_findings}</div>
                <div>Critical Findings</div>
            </div>
            <div class="stat-card">
                <div class="stat-number {"high" if high_findings > 0 else "good"}">{high_findings}</div>
                <div>High Severity</div>
            </div>
            <div class="stat-card">
                <div class="stat-number {"medium" if total_findings > 0 else "good"}">{total_findings}</div>
        ...
        <div class="nav-tabs">
            ...
            <button class="nav-tab" onclick="showTab('security')">🔒 Security Findings</button>
            <button class="nav-tab" onclick="showTab('flows')">🔄 API Flows</button>
            <button class="nav-tab" onclick="showTab('visual')">📈 Visual Maps</button>
        </div>
        
        <div id="overview" class="tab-content active">
            <h2>🎯 Executive Summary</h2>
            <p>This report provides a comprehensive analysis of the API security posture based on traffic analysis.</p>
            
            <h3>🔍 Key Findings</h3>
            <div class="findings-grid">
                <div class="finding-card">
                    <div class="finding-title">Attack Surface</div>
                    <p>{total_endpoints} unique endpoints discovered across {len(set(ep.path.split('/')[1] for ep in intelligence_data.get('endpoint_map', {}).values() if len(ep.path.split('/')) > 1))} services</p>
                </div>
                <div class="finding-card">
                    <div class="finding-title">Security Posture</div>
                    <p>{"🔴 Critical issues found" if critical_findings > 0 else "🟡 Issues require attention" if total_findings > 0 else "🟢 No critical issues detected"}</p>
                </div>
                <div class="finding-card">
                    <div class="finding-title">Authentication</div>
                    <p>{sum(1 for ep in intelligence_data.get('endpoint_map', {}).values() if ep.auth_required)} endpoints require authentication</p>
                </div>
            </div>
            ...
            <div class="export-buttons">
                ...
                <button class="export-btn" onclick="exportReport('markdown')">📝 Export Markdown</button>
            </div>
        </div>
        
        <div id="endpoints" class="tab-content">
            <h2>🎯 Endpoint Analysis</h2>
            <input type="text" class="search-box" placeholder="Search endpoints..." oninput="filterEndpoints(this.value)">
            
            <div class="endpoint-list" id="endpoint-list">
                {self._generate_endpoint_html_list(intelligence_data.get('endpoint_map', {}))}
            </div>
        </div>
        
        <div id="security" class="tab-content">
            <h2>🔒 Security Findings</h2>
            <input type="text" class="search-box" placeholder="Filter findings..." oninput="filterFindings(this.value)">
            
            <div class="findings-grid" id="findings-list">
                {self._generate_findings_html_list(security_findings)}
            </div>
        </div>
        
        <div id="flows" class="tab-content">
            <h2>🔄 API Flow Analysis</h2>
            <p>Authentication flows and business process sequences detected in the traffic.</p>
            
            {self._generate_flows_html_section(flow_data)}
        </div>
        
        <div id="visual" class="tab-content">
            <h2>📈 Visual API Maps</h2>
            
            <h3>API Structure Overview</h3>
            <div class="mermaid">
                {mermaid_diagrams.get('api_structure', '')}
            </div>
            
            <h3>Security Findings Distribution</h3>
            <div class="mermaid">
                {mermaid_diagrams.get('security_overview', '')}
            </div>
            
            <h3>Data Flow Analysis</h3>
            <div class="mermaid">
                {mermaid_diagrams.get('data_flow_diagram', '')}
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {{
            ...
            const cards = document.querySelectorAll('.stat-card, .finding-card, .endpoint-item');
            cards.forEach(card => {{
    ...
    def _generate_endpoint_html_list(self, endpoint_map: Dict) -> str:
        ...
        for path, endpoint in endpoint_map.items():
            ...
            for method in methods:
                ...
                method_badges.append(f'<span class="method-badge {method_class}">{method}</span>')
            
            security_indicator = "🔒" if endpoint.auth_required else "🔓"
            level_color = {"high": "🔴", "medium": "🟡", "low": "🟢"}.get(endpoint.security_level, "⚪")
            
            html_items.append(f'''
                <div class="endpoint-item">
                    <div class="endpoint-path">{security_indicator} {path} {level_color}</div>
                    <div class="endpoint-methods">{''.join(method_badges)}</div>
                    <div><strong>Security Level:</strong> {endpoint.security_level.title()}</div>
                    <div><strong>Auth Required:</strong> {"Yes" if endpoint.auth_required else "No"}</div>
                    <div><strong>Parameters:</strong> {len(endpoint.parameters)} parameters</div>
                    <div><strong>Occurrences:</strong> {endpoint.occurrence_count}</div>
                </div>
            ''')
        
        return ''.join(html_items)
    
    def _generate_findings_html_list(self, security_findings: List[SecurityFinding]) -> str:
        """Generate HTML list of security findings"""
        html_items = []
        
        for finding in security_findings:
            recommendations_html = ""
            if finding.recommendations:
                rec_items = ''.join(f'<li>{rec}</li>' for rec in finding.recommendations)
            ...
            html_items.append(f'''
                <div class="finding-card {finding.severity}">
                    ...
                    <div class="finding-meta">
                        <span class="meta-badge severity-{finding.severity}">{finding.severity.upper()}</span>
                        <span class="meta-badge confidence-badge">Confidence: {finding.confidence:.1%}</span>
                        <span class="meta-badge">Category: {finding.category}</span>
                    </div>
                    <p>{finding.description}</p>
                    <div><strong>Evidence:</strong> <code>{finding.evidence}</code></div>
                    <div><strong>Location:</strong> {finding.location.get('url', 'N/A')}</div>
                    {recommendations_html}
                </div>
            ''')
    ...
    def _generate_flows_html_section(self, flow_data: Dict) -> str:
        """Generate HTML section for flows"""
        auth_flows = flow_data.get('authentication_flows', [])
        business_flows = flow_data.get('business_processes', [])
        
        html_content = []
        
        if auth_flows:
            html_content.append('<h3>🔐 Authentication Flows</h3>')
            for flow in auth_flows:
                steps_html = ''.join(f'<li><strong>{step["method"]}</strong> {step["url"]} → {step["status"]}</li>' 
                                   for step in flow.steps)
                html_content.append(f'''
                    <div class="finding-card">
                        <div class="finding-title">{flow.name}</div>
                        <p><strong>Type:</strong> {flow.flow_type}</p>
                        <p><strong>Authentication Required:</strong> {"Yes" if flow.authentication_required else "No"}</p>
                        <div class="recommendations">
                            <h4>Process Steps:</h4>
                            <ol>{steps_html}</ol>
                        </div>
        ...
        return ''.join(html_content) if html_content else '<p>No significant flows detected in the traffic.</p>'
    
    def _generate_mobile_summary(self, intelligence_data: Dict, security_findings: List[SecurityFinding]) -> str:
        """Generate mobile-friendly summary"""
        total_endpoints = len(intelligence_data.get('endpoint_map', {}))
        critical_findings = sum(1 for f in security_findings if f.severity == 'critical')
        high_findings = sum(1 for f in security_findings if f.severity == 'high')
...
- **{high_findings}** High Priority Issues
- **{len(security_findings)}** Total Findings

## 🔥 Top Priority Actions
'''
        
        # Add top 3 critical findings
        critical_findings_list = [f for f in security_findings if f.severity == 'critical'][:3]
        if critical_findings_list:
            for i, finding in enumerate(critical_findings_list, 1):
                summary += f"\n{i}. **{finding.title}**\n   - {finding.description}\n   - Location: {finding.location.get('url', 'Multiple')}\n"
        else:
            high_findings_list = [f for f in security_findings if f.severity == 'high'][:3]
            for i, finding in enumerate(high_findings_list, 1):
                summary += f"\n{i}. **{finding.title}**\n   - {finding.description}\n   - Location: {finding.location.get('url', 'Multiple')}\n"
        
        # Add high-value targets
        high_value_targets = intelligence_data.get('high_value_targets', [])[:5]
        if high_value_targets:
            summary += "\n## 🎯 High-Value Targets\n"
            for target in high_value_targets:
                summary += f"- **{target['path']}** (Score: {target['score']})\n"
        
        return summary
    
    def _sanitize_id(self, url: str) -> str:
        """Sanitize URL for use as HTML ID"""
        return re.sub(r'[^a-zA-Z0-9]', '_', url.split('/')[-1] or 'root')
...
class ComprehensiveReportGenerator:
    ...
    def generate_comprehensive_reports(self, har_stats: Dict, intelligence_data: Dict, 
                                     flow_data: Dict, security_findings: List[SecurityFinding],
                                     visual_data: Dict, output_dir: str) -> Dict[str, str]:
        ...
        # Executive Summary
        exec_summary = self._generate_executive_summary(har_stats, intelligence_data, security_findings)
        ...
        # Technical Report
        tech_report = self._generate_technical_report(intelligence_data, flow_data, security_findings)
        tech_file = output_path / "technical_report.md"
        with open(tech_file, 'w', encoding='utf-8') as f:
            f.write(tech_report)
        generated_files['technical_report'] = str(tech_file)
        
        # JSON Export
        json_report = self._generate_json_report(har_stats, intelligence_data, flow_data, security_findings)
        json_file = output_path / "complete_analysis.json"
        with open(json_file, 'w', encoding='utf-8') as f:
            json.dump(json_report, f, indent=2, default=str)
        generated_files['json_report'] = str(json_file)
        
        # Interactive HTML Report
        html_file = output_path / "interactive_report.html"
        with open(html_file, 'w', encoding='utf-8') as f:
            f.write(visual_data['html_report'])
        generated_files['html_report'] = str(html_file)
        
        # Mobile Summary
        mobile_file = output_path / "mobile_summary.md"
        with open(mobile_file, 'w', encoding='utf-8') as f:
            f.write(visual_data['mobile_summary'])
        generated_files['mobile_summary'] = str(mobile_file)
        
        return generated_files
    
    def _generate_executive_summary(self, har_stats: Dict, intelligence_data: Dict, 
                                   security_findings: List[SecurityFinding]) -> str:
        ...
        risk_level = "CRITICAL" if critical_findings > 0 else "HIGH" if high_findings > 0 else "MODERATE"
        
        summary = f'''# 🎯 Executive Summary - API Security Assessment

## Overview
This assessment analyzed **{har_stats.get('total_requests', 0)} HTTP requests** across **{har_stats.get('unique_domains', 0)} domains**, identifying **{total_endpoints} unique API endpoints** and **{len(security_findings)} security findings**.
...
- **High Priority Issues:** {high_findings}
- **High-Value Targets:** {len(high_value_targets)}
...
'''
        ...
        if critical_findings > 0:
            ...
            critical_list = [f for f in security_findings if f.severity == 'critical'][:3]
            for finding in critical_list:
                summary += f"- **{finding.title}**: {finding.description}\n"
        
        if high_findings > 0:
            summary += "\n#### 🟡 HIGH PRIORITY\n"
            high_list = [f for f in security_findings if f.severity == 'high'][:3]
            for finding in high_list:
                summary += f"- **{finding.title}**: {finding.description}\n"
        
        summary += f'''

### Attack Surface Analysis
- **Authenticated Endpoints:** {sum(1 for ep in intelligence_data.get('endpoint_map', {}).values() if ep.auth_required)}
- **Public Endpoints:** {sum(1 for ep in intelligence_data.get('endpoint_map', {}).values() if not ep.auth_required)}
- **Admin Interfaces:** {sum(1 for target in high_value_targets if 'admin' in target.get('reasons', []))}

### Recommendations
1. **Immediate**: Address all critical security findings
2. **Short-term**: Implement missing security headers and controls
3. **Medium-term**: Enhance authentication and authorization mechanisms
4. **Long-term**: Establish continuous API security monitoring

### Next Steps
1. Review detailed technical findings in the full report
2. Prioritize remediation based on risk scores
3. Implement security controls for high-value targets
4. Establish ongoing security testing practices
...
'''
    ...
    def _generate_technical_report(self, intelligence_data: Dict, flow_data: Dict, 
                                  security_findings: List[SecurityFinding]) -> str:
        """Generate detailed technical report"""
        endpoint_map = intelligence_data.get('endpoint_map', {})
        api_patterns = intelligence_data.get('api_patterns', {})
        versioning = intelligence_data.get('versioning', {})
        
        report = f'''# 🔧 Technical Security Analysis Report

## API Architecture Analysis

### Endpoint Distribution
- **Total Endpoints:** {len(endpoint_map)}
- **REST Patterns:** {api_patterns.get('rest_patterns', {}).get('GET_api', 0)} GET endpoints
- **Authentication Required:** {sum(1 for ep in endpoint_map.values() if ep.auth_required)} endpoints
- **High Security Level:** {sum(1 for ep in endpoint_map.values() if ep.security_level == 'high')} endpoints

### API Versioning Strategy
**Strategy Detected:** {versioning.get('versioning_strategy', 'unknown').replace('_', ' ').title()}
'''
        
        if versioning.get('versions_detected'):
            report += f"**Versions Found:** {', '.join(sorted(versioning['versions_detected']))}\n"
...
'''
        ...
        for category, findings in findings_by_category.items():
            report += f"#### {category.replace('_', ' ').title()} ({len(findings)} findings)\n\n"
            
            for finding in findings[:5]:  # Limit to top 5 per category
                report += f"**{finding.title}** (Severity: {finding.severity}, Confidence: {finding.confidence:.1%})\n"
                report += f"- Description: {finding.description}\n"
                report += f"- Location: {finding.location.get('url', 'Multiple endpoints')}\n"
                
                if finding.recommendations:
                    report += f"- Recommendations: {finding.recommendations[0]}\n"
                report += "\n"
...
The following {len(high_value_targets)} endpoints require focused security testing:

'''
            for target in high_value_targets[:10]:
                reasons = ', '.join(target['reasons'])
                report += f"**{target['path']}** (Score: {target['score']})\n"
                report += f"- Methods: {target['methods']}\n"
                report += f"- Risk Factors: {reasons}\n"
                report += f"- Parameters: {len(target['parameters'])} parameters\n\n"
...
{len(auth_flows)} authentication flows were identified:

'''
            for flow in auth_flows:
                report += f"**{flow.name}** ({len(flow.steps)} steps)\n"
                for step in flow.steps:
                    report += f"  {step['step']}. {step['method']} {step['url']} → {step['status']}\n"
                report += "\n"
        
        # Parameter analysis
        param_intel = intelligence_data.get('parameter_intelligence', {})
        if param_intel.get('common_parameters'):
            report += '''
### Parameter Analysis

Most common parameters detected:
'''
            common_params = param_intel['common_parameters'].most_common(10)
            for param, count in common_params:
                report += f"- **{param}**: {count} occurrences\n"
        
        report += f'''

### Wordlist Generation

Generated wordlists for further testing:
- **Discovered Endpoints**: {len(intelligence_data.get('wordlists', {}).get('discovered_endpoints', []))} entries
- **Extrapolated Endpoints**: {len(intelligence_data.get('wordlists', {}).get('extrapolated_endpoints', []))} entries  
- **Parameter Names**: {len(intelligence_data.get('wordlists', {}).get('parameter_names', []))} entries
- **Admin Candidates**: {len(intelligence_data.get('wordlists', {}).get('admin_candidates', []))} entries

### Testing Recommendations

1. **Immediate Testing**: Focus on high-value targets with scores > 50
2. **Authentication Testing**: Test all identified auth flows for bypass opportunities
3. **Parameter Testing**: Use generated parameter lists for injection testing
4. **Endpoint Discovery**: Use extrapolated endpoints for comprehensive discovery
...
'''
    ...
    def _generate_json_report(self, har_stats: Dict, intelligence_data: Dict, 
                             flow_data: Dict, security_findings: List[SecurityFinding]) -> Dict:
        """Generate comprehensive JSON export"""
        return {
            'metadata': {
                'generated_at': datetime.now().isoformat(),
                'tool_version': '1.0.0',
                'analysis_type': 'comprehensive_api_security'
            },
            'statistics': har_stats,
            'intelligence': {
                'endpoint_map': {path: asdict(endpoint) for path, endpoint in intelligence_data.get('endpoint_map', {}).items()},
                'api_patterns': intelligence_data.get('api_patterns', {}),
                'versioning': intelligence_data.get('versioning', {}),
                'parameter_intelligence': intelligence_data.get('parameter_intelligence', {}),
                'high_value_targets': intelligence_data.get('high_value_targets', []),
                'attack_surface_summary': intelligence_data.get('attack_surface_summary', {})
            },
            'flows': {
                'authentication_flows': [asdict(flow) for flow in flow_data.get('authentication_flows', [])],
                'business_processes': [asdict(flow) for flow in flow_data.get('business_processes', [])],
                'data_flows': flow_data.get('data_flows', {}),
                'error_patterns': flow_data.get('error_patterns', {})
            },
            'security_findings': [asdict(finding) for finding in security_findings],
            'wordlists': intelligence_data.get('wordlists', {}),
            'recommendations': self._generate_prioritized_recommendations(security_findings, intelligence_data)
        }
    
    def _generate_prioritized_recommendations(self, security_findings: List[SecurityFinding], 
                                            intelligence_data: Dict) -> List[Dict]:
        ...
        if critical_findings:
            recommendations.append({
                ...
                'description': f'{len(critical_findings)} critical issues require immediate attention',
                'actions': [rec for finding in critical_findings for rec in finding.recommendations]
            })
        
        # High-value targets recommendations
        high_value_targets = intelligence_data.get('high_value_targets', [])
        if high_value_targets:
            recommendations.append({
                'priority': 'high',
                'category': 'focused_testing',
                'title': 'Test High-Value Targets',
                'description': f'Focus security testing on {len(high_value_targets)} high-value endpoints',
                'actions': [
                    'Conduct manual penetration testing on admin interfaces',
                    'Test direct object references for IDOR vulnerabilities',
        ...
        wordlists = intelligence_data.get('wordlists', {})
        if wordlists.get('discovered_endpoints'):
            recommendations.append({
                'priority': 'medium',
                'category': 'endpoint_discovery',
                'title': 'Comprehensive Endpoint Discovery',
                'description': 'Use generated wordlists for thorough endpoint enumeration',
                'actions': [
                    f"Use {len(wordlists['discovered_endpoints'])} discovered endpoints for fuzzing",
                    f"Test {len(wordlists.get('extrapolated_endpoints', []))} extrapolated endpoints",
                    'Verify access controls on discovered admin endpoints'
                ]
            })
...
def main():
    """Enhanced main CLI interface with complete functionality"""
    parser = argparse.ArgumentParser(description="Complete API Security Analysis Tool (Stages 1-5)")
    parser.add_argument("har_file", help="Path to HAR file")
    parser.add_argument("--output", "-o", help="Output directory", default="./api_security_analysis")
    parser.add_argument("--export-all", help="Export all formats", action="store_true")
    ...
    parser.add_argument("--export-html", help="Export interactive HTML", action="store_true")
    parser.add_argument("--open-browser", help="Open HTML report in browser", action="store_true")
    parser.add_argument("--quiet", "-q", help="Minimal output", action="store_true")
    parser.add_argument("--stage", help="Run specific stage only (1-5)", type=int, choices=[1,2,3,4,5])
    
    args = parser.parse_args()
    
    if not args.quiet:
        console.print(Panel.fit(
            "[bold cyan]🎯 API Security Analysis Tool - Complete Suite[/bold cyan]\n"
            "[yellow]Stages 1-5: HAR Processing → Security Scanning → Intelligence → Flow Analysis → Visual Mapping[/yellow]\n"
            "[green]Ready to transform traffic into actionable security intelligence[/green]",
            border_style="cyan"
        ))
    ...
    har_processor = AdvancedHARProcessor(args.har_file)
    security_scanner = EnhancedSecurityScanner()
    intelligence_engine = EndpointIntelligenceEngine()
    flow_analyzer = FlowAnalysisEngine()
    visual_mapper = VisualMappingEngine()
    report_generator = ComprehensiveReportGenerator()
    
    # Stage 1: Enhanced HAR Processing
    if not args.stage or args.stage >= 1:
        console.print("[cyan]🔧 Stage 1: Enhanced HAR Processing[/cyan]")
        if not har_processor.load_har():
            return 1
        
        requests = har_processor.extract_requests_with_relationships()
        if not requests:
            console.print("[red]✗[/red] No requests found in HAR file")
            return 1
        
        console.print(f"[green]✓[/green] Processed {len(requests)} requests with relationship detection")
    
    # Stage 2: Enhanced Security Scanning
    if not args.stage or args.stage >= 2:
        console.print("[cyan]🔍 Stage 2: Enhanced Security Scanning[/cyan]")
        security_findings = security_scanner.scan_requests_enhanced(
            requests, har_processor.request_relationships
        )
        console.print(f"[green]✓[/green] Identified {len(security_findings)} security findings")
    
    # Stage 3: Endpoint Intelligence
    if not args.stage or args.stage >= 3:
        console.print("[cyan]🎯 Stage 3: Endpoint Intelligence Analysis[/cyan]")
        intelligence_data = intelligence_engine.analyze_endpoint_intelligence(
            requests, har_processor.request_relationships
        )
        console.print(f"[green]✓[/green] Analyzed {len(intelligence_data['endpoint_map'])} unique endpoints")
    
    # Stage 4: Flow Analysis
    if not args.stage or args.stage >= 4:
        console.print("[cyan]🔄 Stage 4: API Flow Analysis[/cyan]")
        flow_data = flow_analyzer.analyze_api_flows(
            requests, har_processor.request_relationships, security_findings
        )
        auth_flows = len(flow_data.get('authentication_flows', []))
        business_flows = len(flow_data.get('business_processes', []))
        console.print(f"[green]✓[/green] Detected {auth_flows} auth flows and {business_flows} business processes")
    
    # Stage 5: Visual Mapping
    if not args.stage or args.stage >= 5:
        console.print("[cyan]🎨 Stage 5: Visual Mapping & Reports[/cyan]")
        visual_data = visual_mapper.generate_visual_maps(
            intelligence_data, flow_data, security_findings
        )
        console.print("[green]✓[/green] Generated interactive visual maps and reports")
    ...
    if args.export_all or any([args.export_json, args.export_html, args.export_wordlists]):
        generated_files = report_generator.generate_comprehensive_reports(
            har_processor.stats, intelligence_data, flow_data, 
            security_findings, visual_data, str(output_path)
        )
        
        console.print(f"\n[green]✓[/green] Reports generated in: {output_path}")
        for report_type, file_path in generated_files.items():
            console.print(f"  📄 {report_type}: {Path(file_path).name}")
    
    # Export wordlists
    if args.export_wordlists or args.export_all:
        ...
        for name, wordlist in wordlists.items():
            if wordlist:
                file_path = wordlist_dir / f"{name}.txt"
                with open(file_path, 'w') as f:
                    f.write('\n'.join(wordlist))
                console.print(f"[green]✓[/green] Wordlist: {file_path.name} ({len(wordlist)} entries)")
    ...
    if not args.quiet:
        ...
        summary_table = Table(title="📊 Final Analysis Summary", show_header=True, header_style="bold cyan")
        summary_table.add_column("Metric", style="cyan", width=25)
        summary_table.add_column("Count", style="green", justify="right", width=10)
        summary_table.add_column("Details", style="yellow", width=40)
        
        summary_table.add_row("Requests Processed", str(len(requests)), f"{har_processor.stats.get('unique_domains', 0)} domains")
        summary_table.add_row("Endpoints Discovered", str(len(intelligence_data.get('endpoint_map', {}))), f"{sum(1 for ep in intelligence_data.get('endpoint_map', {}).values() if ep.auth_required)} authenticated")
        summary_table.add_row("Security Findings", str(len(security_findings)), f"{sum(1 for f in security_findings if f.severity == 'critical')} critical")
        summary_table.add_row("High-Value Targets", str(len(intelligence_data.get('high_value_targets', []))), "Prioritized for testing")
        summary_table.add_row("Auth Flows", str(len(flow_data.get('authentication_flows', []))), "Authentication sequences")
        summary_table.add_row("Business Flows", str(len(flow_data.get('business_processes', []))), "Process workflows")
        
        console.print(summary_table)
        
        # Show top findings
        if security_findings:
            console.print(f"\n[bold red]🔥 TOP PRIORITY FINDINGS:[/bold red]")
            for finding in security_findings[:3]:
                severity_color = {"critical": "red", "high": "yellow", "medium": "blue"}.get(finding.severity, "white")
                console.print(f"[{severity_color}]•[/{severity_color}] [{severity_color}]{finding.title}[/{severity_color}] ({finding.severity})")
                console.print(f"   {finding.description}")
        
        # Processing time
        elapsed_time = time.time() - start_time
        console.print(f"\n[green]✓[/green] Complete analysis finished in {elapsed_time:.2f} seconds")
    
    # Open browser if requested
    if args.open_browser and (args.export_html or args.export_all):
        html_file = output_path / "interactive_report.html"
        if html_file.exists():
...
if __name__ == "__main__":
    sys.exit(main())finding-card">
                        <div class="finding-title">{flow.name}</div>
                        <p><strong>Steps:</strong> {len(flow.steps)}</p>
                        <div class="recommendations">
                            <h4>Flow Steps:</h4>
                            <ol>{steps_html}</ol>
                        </div>
        ...
        if business_flows:
            html_content.append('<h3>💼 Business Process Flows</h3>')
            for flow in business_flows:
                steps_html = ''.join(f'<li><strong>{step["method"]}</strong> {step["url"]}</li>' 
                                   for step in flow.steps)
                html_content.append(f'''
                    <div class="